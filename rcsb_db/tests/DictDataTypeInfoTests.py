# File:    DictDataTypeInfoTests.py.py
# Author:  J. Westbrook
# Date:    22-May-2013
# Version: 0.001
#
...
from rcsb_db.schema.DictDataTypeInfo import DictDataTypeInfo
from rcsb_db.utils.IoUtil import IoUtil


class DataTypeInfoTests(unittest.TestCase):
    ...
    def testDefaults(self):
        ...
        """
        try:
            dtInfo = DictDataTypeInfo(filePath=None, applicationName='ANY', workPath=None)
            mapD = dtInfo.getDefaultDataTypeMap()
            logger.info("Default type map length %d" % len(mapD))
            ok = self.__ioU.serializeJson(self.__pathSaveTypeMapJson, mapD, indent=3)
            self.assertTrue(ok)
            ...
            self.assertDictEqual(uMapD, rMapD)
            #
        except Exception as e:
...
def dictTypeInfoDefaultSuite():
    ...
    suiteSelect.addTest(DataTypeInfoTests("testDefaults"))
    return suiteSelect
