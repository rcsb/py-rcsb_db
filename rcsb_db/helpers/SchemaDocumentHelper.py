...
from rcsb_db.helpers.SchemaDocumentHelperBase import SchemaDocumentHelperBase
...
class SchemaDocumentHelper(SchemaDocumentHelperBase):
    ...
    __collection_attribute_name = {'pdbx_v5_0_2': 'entry.id',
    ...
    def getDocumentKeyAttributeName(self, collectionName):
        r = (None, None)
        try:
            return SchemaDocumentHelper.__collection_attribute_name[collectionName]
        except Exception as e:
            logger.debug("Collection %s failing with %s" % (collectionName, str(e)))
        return r
