##
# File:    SchemaDefPdbxLoaderCrateDbMultiTests.py
# Author:  J. Westbrook
# Date:    19-Jan-2018
# Version: 0.001
#
...
and external schema definitions using CrateDb services -   This version divides parsing
...
from rcsb_db.loaders.SchemaDefCrateDbLoader import SchemaDefLoader
...
class SchemaDefLoaderCrateDbMultiTests(unittest.TestCase):
    ...
    def loadInsertMany(self, dataList, procName, optionsD, workingDir):
        ...
        try:
            ...
            myC = CrateDbConnect(dbHost=self.__dbHost, dbPort=self.__dbPort)
            dbCon = myC.connect()
            sd = optionsD['sd']
            skipD = optionsD['skip']
    ...
    def __schemaCreate(self, schemaDefObj):
        ...
        try:
            ...
            sqlGen = MyDbAdminSqlGen(self.__verbose,  serverType="cratedb")
            sqlL = []
            ...
            myC = CrateDbConnect(dbHost=self.__dbHost, dbPort=self.__dbPort)
            dbCon = myC.connect()
            crQ = CrateDbQuery(dbcon=dbCon, verbose=self.__verbose)
...
def loadSuite():
    ...
    suiteSelect.addTest(SchemaDefLoaderCrateDbMultiTests("testLoadPdbxAllMulti"))
    return suiteSelect
