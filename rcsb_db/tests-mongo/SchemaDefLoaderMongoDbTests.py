...
##
"""
Tests for creating and loading distributed document database using PDBx/mmCIF data files
and external schema definitions using MongoDb services -  Covers BIRD, CCD and PDBx/mmCIF
model files -
...
from rcsb_db.mongo.MongoDbUtilimport MongoDbConnect, MongoDbQuery
...
class SchemaDefLoaderMongoDbTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # default database
        #
        self.__dbHost = os.getenv("Mongo_DB_HOST", 'localhost')
        self.__dbPort = os.getenv("Mongo_DB_PORT", 26257)
        self.__dbName = os.getenv("Mongo_DB_NAME", None)
        self.__dbUser = os.getenv("Mongo_DB_USER", 'root')
        #
    ...
    def open(self, dbName, dbUser, dbHost, dbPort):
        self.close()
        try:
            myC = MongoDbConnect(dbName=dbName, dbUser=dbUser, dbHost=dbHost, dbPort=dbPort)
    ...
    def testClusterConnections(self):
        try:
            clusterHostList = str(os.getenv("Mongo_DB_CLUSTER_HOSTS", "")).split(',')
            #
    ...
    def testLoadInsertBirdReference(self):
        ...
        try:
            ...
            ret = sdl.load(inputPathList=inputPathList, loadType='Mongo-insert', deleteOpt='selected')
            self.assertEqual(ret, True)
        except Exception as e:
            logger.exception("Failing with %s" % str(e))
            self.fail()
    ...
    def testLoadInsertManyBirdReference(self):
        try:
            ...
            sdl = SchemaDefLoader(schemaDefObj=sd, ioObj=self.__ioObj, dbCon=self.__dbCon, workPath='.',
                                  cleanUp=False, warnings='default', verbose=self.__verbose)
            ret = sdl.load(inputPathList=inputPathList, loadType='Mongo-insert-many', deleteOpt='selected')
            self.assertEqual(ret, True)
        except Exception as e:
            logger.exception("Failing with %s" % str(e))
            self.fail()
    ...
    def testLoadInsertPdbxExampleFiles(self):
        try:
            ...
            sdl = SchemaDefLoader(schemaDefObj=sd, ioObj=self.__ioObj, dbCon=self.__dbCon, workPath='.', cleanUp=False, warnings='default', verbose=self.__verbose)
            ret = sdl.load(inputPathList=inputPathList, loadType='Mongo-insert', deleteOpt='selected', tableIdSkipD=self.__tableIdSkipD)
            self.assertEqual(ret, True)
        except Exception as e:
            logger.exception("Failing with %s" % str(e))
            self.fail()
    ...
    def testLoadInsertManyPdbxExampleFiles(self):
        try:
            ...
            inputPathList = self.__getPdbxPathList()
            sdl = SchemaDefLoader(schemaDefObj=sd, ioObj=self.__ioObj, dbCon=self.__dbCon, workPath='.', cleanUp=False, warnings='default', verbose=self.__verbose)
            ret = sdl.load(inputPathList=inputPathList, loadType='Mongo-insert-many', deleteOpt='selected', tableIdSkipD=self.__tableIdSkipD)
            self.assertEqual(ret, True)
        except Exception as e:
            logger.exception("Failing with %s" % str(e))
            self.fail()
    ...
    def __schemaCreateSQL(self, schemaDefObj):
        ...
        try:
            ...
            sqlGen = MyDbAdminSqlGen(self.__verbose, self.__lfh, serverType="MongoDb")
    ...
    def __schemaCreate(self, schemaDefObj):
        """Test case -  create Mongo DB and collection --
        """
        ret = 0
        try:
            tableIdList = schemaDefObj.getTableIdList()
            ...
            #
            crQ = MongoDbQuery(dbcon=self.__dbCon, verbose=self.__verbose)
...
def createConnectionSuite():
    ...
    suiteSelect.addTest(SchemaDefLoaderMongoDbTests("testConnection"))
    # suiteSelect.addTest(SchemaDefLoaderMongoDbTests("testClusterConnections"))
    return suiteSelect


def removeSchemaSuite():
    suiteSelect = unittest.TestSuite()
    suiteSelect.addTest(SchemaDefLoaderMongoDbTests("testSchemaRemove"))
    return suiteSelect


def createSchemaSuite():
    suiteSelect = unittest.TestSuite()
    suiteSelect.addTest(SchemaDefLoaderMongoDbTests("testSchemaCreateSQL"))
    suiteSelect.addTest(SchemaDefLoaderMongoDbTests("testSchemaCreate"))
    return suiteSelect


def loadBirdReferenceSuite():
    suiteSelect = unittest.TestSuite()
    suiteSelect.addTest(SchemaDefLoaderMongoDbTests("testLoadInsertBirdReference"))
    suiteSelect.addTest(SchemaDefLoaderMongoDbTests("testLoadInsertManyBirdReference"))
    return suiteSelect


def loadCCReferenceSuite():
    suiteSelect = unittest.TestSuite()
    suiteSelect.addTest(SchemaDefLoaderMongoDbTests("testLoadInsertChemCompReference"))
    suiteSelect.addTest(SchemaDefLoaderMongoDbTests("testLoadInsertManyChemCompReference"))
    return suiteSelect


def loadPdbxSuite():
    suiteSelect = unittest.TestSuite()
    suiteSelect.addTest(SchemaDefLoaderMongoDbTests("testLoadInsertPdbxExampleFiles"))
    suiteSelect.addTest(SchemaDefLoaderMongoDbTests("testLoadInsertManyPdbxExampleFiles"))
    return suiteSelect
