...
class CrateDbConnect(object):

    """ Class to encapsulate RDBMS DBI connection.
    """

    def __init__(self, dbHost='localhost', dbPort=4200, verbose=False):
        self.__verbose = verbose

        if (dbHost is None):
            self.__dbHost = os.getenv("CRATE_DB_HOST")
        else:
            self.__dbHost = dbHost

        if dbPort is None:
            # try from the environment -
            tS = os.getenv("CRATE_DB_PORT")
            if (tS is not None):
    ...
    def setAuth(self, authD):
        try:
            ...
            # treat port as optional with default of 4200
            if 'DB_PORT' in authD:
                self.__dbPort = authD["DB_PORT"]
            else:
                self.__dbPort = 4200
        except Exception as e:
            logger.exception(" failing  %r" % authD.items())
